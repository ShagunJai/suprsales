...
$.ajax({
    ...
    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
	  ...
	  success:function(data){
			  ...
			  location.href = 'http://apps.insecticidesindia.com:8030/delegatedtask/'+task_id;
...
$("body").on("click", ".activity_detail", function(event){   
		...
		var dateString2 = new Date(date2.getTime() - (date2.getTimezoneOffset() * 60000 ))
			...
			for(var i=0; i<auths.ATTACHMENT.length; i++){
				var path = "http://apps.insecticidesindia.com:8030/storage/task/"+task_id+"/"+auths.ACTIVITY_ID+"/"+auths.ATTACHMENT[i].ACTIVITY_ATTACHMENT;
			...
			for(var i=0; i<auths.ACTIVITY_OWNER.length; i++){
				if(auths.ACTIVITY_OWNER[i].ACTIVITY_OWNER_IMAGE == null){
					...
					var path = "http://apps.insecticidesindia.com:8030/storage/"+auths.ACTIVITY_OWNER[i].ACTIVITY_OWNER_IMAGE;
			...
			for(var i=0; i<auths.ACTIVITY_DELIGATION.length; i++){
				...
				if(auths.ACTIVITY_DELIGATION[i].DELIGATION_FROM_IMAGE == null){
					...
					var path = "http://apps.insecticidesindia.com:8030/storage/"+auths.ACTIVITY_DELIGATION[i].DELIGATION_FROM_IMAGE;
			...
			for(var i=0; i<auths.ACTIVITY_DELIGATION.length; i++){	
				if(auths.ACTIVITY_DELIGATION[i].DELIGATED_TO_IMAGE == null){
					...
					var path = "http://apps.insecticidesindia.com:8030/storage/"+auths.ACTIVITY_DELIGATION[i].DELIGATED_TO_IMAGE;
