...
<head>
 ...
 <!-- fullCalendar -->
  <link href="{!! asset('suprsales_resource/plugins/fullcalendar/main.min.css') !!}" rel="stylesheet" type="text/css">
  <link href="{!! asset('suprsales_resource/plugins/fullcalendar-daygrid/main.min.css') !!}" rel="stylesheet" type="text/css">
  <link href="{!! asset('suprsales_resource/plugins/fullcalendar-timegrid/main.min.css') !!}" rel="stylesheet" type="text/css">
  <link href="{!! asset('suprsales_resource/plugins/fullcalendar-bootstrap/main.min.css') !!}" rel="stylesheet" type="text/css">
  <!-- Theme style -->
...
{{-- Import auth user by id and it should not be null --}}
   @if(isset(Auth::user()->id))
  <div class="wrapper">

    {{-- store the respective email inside the $user --}}
    @php
   ...
   {{--A flash message allows you to create a message on one page and display it once on another page.--}}
      ...
      @endif
   {{--A flash message allows you to create a error message --}}
   ...
   {{--A flash message allows you to create a information --}}
      @if(Session::has('info'))
      <script>
        toastr.options = {
          "closeButton": true,
   ...
   {{--A flash message allows you to create a warning message --}}
...
<script>
  ...
  <script>
    //   it will show the plant by define the id
    //  this to set or return the attributes and values of the selected elements.
    $("body").on("click", ".plant", function(event) {
      var auth = $(this).attr("id");
      //getElementById() method returns the element that has the ID attribute with -
    ...
    //parse() method parses a JSON string, constructing the JavaScript value or object described by the string and store the value inside auths
      var auths = JSON.parse(values);


    // store the plant id get from the auths
      $('#plant_Id').val(auths.PLANT_ID);
    //   store the plant_name get from the auths
      $('#plant_name').val(auths.PLANT_DESCRIPTION);

// then if the compare the flag value inside auths with 1 or not
// bootstrapSwitch() is use to Set the state as off and do not trigger switchChange event.
      if (auths.FLAG == '1') {
...
//  this is to show the model as dropdown but not a dropdown something
      $('.plantModal').modal("show");
    });

// for updation of plant this is the plant form
    $('#plant_form').on("submit", function(event) {
// preventDefault() method is used to prevent the browser from executing the default action of the selected element
      event.preventDefault();
// store the flag value and the plantview value Its easy  to get the values of input fiedls
      var plantview = document.getElementById("plant_Id").value;
      var flag = document.getElementById("flag").value;
//  ajax it is the use of the XMLHttpRequest object to communicate with servers.
      $.ajax({
        type: 'POST',
        url: '/plantview/' + plantview,

// CSRF tokens normally go in a form as hidden form fields. Putting them in a meta tag only makes sense
   headers: {
        ...
        // it give the updates successful message
  ...
  <script>
    ...
    //   it gives the region zone by their id
    var region_zone;

    function region_zones(e) {
     // it used to target event property returns the element that triggered the event.
      ...
      for (var j = 0; j < regions.length; j++) {
        ...
        //  it shows tthe region name with their id
    ...
    // this is use for create retailer
    var createretailer;
...
function myBeatPlan(e) {
 
var reptsplann = document.getElementById("rept_id").value;
//console.log(reptsplann);
beatplandate = e.target.value;
//console.log(beatplandate);
var pqr=reptsplann;

$.ajax({
url: '/createbeatplan/' + reptsplann + '/' + beatplandate + '/' + pqr,
type: 'get',
dataType: 'json',
success: function(response) {
...
for (var i = 0; i < len; i++) {
 ...
 var obj = response['data'][i].CUSTOMER_ID;
  arr.push(obj);
}

$("#custs_id").select2().val(arr).trigger("change");
}
 ...
 $(document).ready(function () {
  ...
  $("body").on("click", ".edit_beat_plan", function(event) {
      var reptsplan = $(this).attr("id");
      var vals = document.getElementById(reptsplan).getAttribute("value");
      var obj = JSON.parse(vals);
       //It shows the individual data to the UI as an object
       
       var custobj=obj.cust_array
      // console.log(custobj);
      $("#custs_id").empty();
       $('#rept_id').val(obj.EMP_ID);
       // $('#employees_id').val(obj.EMP_ID);
        $('#rept_name').val(obj.EMP_NAME);
        //$('#assign_Id').val(obj.EMP_ID);
        var arry=[];
        var arryy=[]
        for(var p=0;p<custobj.length;p++){
           var cstid=custobj[p].CUSTOMER_ID;
           var cstname=custobj[p].CUSTOMER_NAME;
           arry.push(cstid);
           arryy.push(cstname);
           $("#custs_id").append("<option value='" + cstid + "'>" + cstname+ "</option>");
        }
        
        
    
       
$('.BeatPlanModel').modal("show");
 ...
 $('#editBeatPlan_form').on("submit", function(event) {
       event.preventDefault();
       var reptsid = document.getElementById("rept_id").value;
        //alert(reptsid);
        updatedate=document.getElementById("beat_plan_date").value;       
        //alert(updatedate);
        // var input = document.getElementsByName('custs_id[]');
        // console.log(input);
       
        var selectedCustId = $('#custs_id').select2("val");
         //It is for put the data in the respective variable inside empss as EmpController inside EditEmp
          if(selectedCustId == 0 || selectedCustId== null){
            toastr.error('Beat Plan Not Updated');
            location.reload();
          }else{
         //console.log(selected);
         $.ajax({
        type: 'POST',
        url: '/createbeatplan/' + updatedate + '/' + reptsid,
        headers: {
          'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        // PUT is used to send data to a server to create/update a resource.
        ...
        success: function(data){
          toastr.success('Update Beat Plan Successfully')
...
// it store the value define by their data lengh
          if (len > 0) {
            for (var i = 0; i < len; i++) {
              var dis = response['data'][i].AREA_NAME;
              var diss = response['data'][i].AREA_ID;
...
// it store the value define by their data lengh if the len value >0
          if (len > 0) {
            for (var i = 0; i < len; i++) {
              var dis = response['data'][i].AREA_NAME;
              var diss = response['data'][i].AREA_ID;
...
// it store the value define by their data lengh if the leng value >0
          if (leng > 0) {
  ...
  <script>
    ...
    //   it shos the admin type as he is depot manager or org admin define by the option value
    var admintype;
  ...
  <script>
    //   it will show the Assign Customer define by the employee id
  ...
  <script>
    ...
    //   It check the flag value throughout the project
...
<script>
  ...
  <script>
    //   It is used to specify the function to run after the document is loaded.
    $(document).ready(function() {
      $('table.display').DataTable({
        //  specify whether the paging of the DataTable is enabled
        "paging": true,
        //specify the number of rows of the table that are to be displayed on one page
        "pageLength": 4,
        //specify the options that are available in the drop-down list that controls the number of rows that a single page will have when pagination is enabled.
        "lengthMenu": [
          [4, 10, -1],
          [4, 10, "All"]
        ],
        // specify whether the dropdown to change the number of rows per page is displayed or not
        "lengthChange": true,
        //to help find the item of data you are looking for.
        "searching": true,
        //enable the order
        "ordering": true,
        "info": true,
        //specify whether the automatic calculation of the column width in the DataTable is enabled or not.
        "autoWidth": false,
        "responsive": true,
        // columns to be defined one or more times in different column definition objects
        'columnDefs': [{
            'targets': 0,
            className: 'dt-center'
          },
...
<script>
  ...
  <script>
    $(function() {
      //Initialize Select2 Elements through out the program
      $('.select2').select2()
...
$(document).ready(function (){
  ...
  var tle;
// this shows the table data  as a table as excel file
      tle = $('#createorder').DataTable({
        "paging": true,
        "pageLength": 100000,
...
//   it  is used to specify the function that will be invoked when the DataTable has fully loaded all the data.
        initComplete: function() {
          ...
          btns.removeClass('dt-button');
            ...
            $("div.toolbar")
            .html(' <button type="submit" class="btn btn-info " id="submitbutton">Submit</button>');
            ...
            $('#submitbutton').on('click', function(e) {
              ...
              var skuu_id = tle.rows({
              selected: true
            }).data().pluck(2).toArray();
           // alert(skuu_id);
         ...
         var ss= qnt.length;
            // alert(ss);
            // alert(qnt);
              let total_skuu = tle.rows({
              selected: true
            }).data().pluck(6).toArray();
            let factor = tle.rows({
              selected: true
            }).data().pluck(9).toArray();
            //alert(factor)
            let Total_sku_value=[];
            for (var i = 0; i <Math.min(qnt.length,total_skuu.length,factor.length); i++) 
          {

            Total_sku_value[i]=qnt[i]*total_skuu[i]*factor[i];
    
          }
          
         // alert(Total_sku_value);
       
           let Total_order_value = Total_sku_value.reduce(function (a, b) 
           {
            return a + b;
          }, 0);
           // alert(Total_order_value); 
         //console.log(qnt);
         //alert(qnt);
         var delay = 4000;
        // alert(delay);
...
//   it  is used to specify the function that will be invoked when the DataTable has fully loaded all the data.
        initComplete: function() {
          ...
          // var approve=document.getElementById('approve_status').value;
          var status = new Array();
           ...
           status.push($(this).val());
            ...
            const approve = status.filter(el => {
           return  el == '2' ;
             });  

             var acknowledge = new Array();
           $("input[name=acknowledge_status]").each(function() {
            
            acknowledge.push($(this).val());
  
            });
            const acknowle = acknowledge.filter(el => {
           return  el == '1' ;
             });  
          ...
          //alert(status);
            $("div.toolbar")
            .html(' <button type="submit" class="btn btn-info " id="submitbutton" >Submit</button><div><p id="message" ></p></div>');
           if(approve=='2'){
            document.getElementById("submitbutton").disabled = true;

           
            document.getElementById("message").innerHTML=("<span style='color: red;'>New Order Cannot Be Created Untill Previous Order Is Acknowledged or Pending!!");
            toastr.warning('Your Previous  Order Is Pending')
           }
           else{
           ...
           if(acknowle=='1'){
            document.getElementById("submitbutton").disabled = true;
            document.getElementById("message").innerHTML=("<span style='color: red;'>New Order Cannot Be Created Untill Previous Order Is Acknowledged!!");
            ...
            //var qnt=$(col).find("input").val();
            //var qnt=$("#createorder tr.t-state-selected:seventh-child(7)input");
...
$('#customerorder').DataTable({
        ...
        initComplete: function() {
          ...
          btns.removeClass('dt-button');
          $("div.toolbar")
            //.html('<a class="btn btn-info btn-sm" href="http://127.0.0.1:8000/createorder" title="Create"><i class="fas fa-user-plus"></i> Create</a>');
      ...
      $("body").on("click", ".allord", function(event) {
       order = $(this).attr("id");

        var order_split = order.split("_");
        var customer = order_split[0];
         ord_id = order_split[1];
        //alert(ord_id);
        //alert(customer);

       // var custallorder='1012245'; 
        if (ord_id=='1' || ord_id=='4'){
          document.getElementById('acknowledge_button').disabled=false;
          document.getElementById('acknowledge_button').hidden=false;

        }
        else{
          document.getElementById('acknowledge_button').disabled=true;
          document.getElementById('acknowledge_button').hidden=true;

        }

        $('#total_vals').html(customer);
        $('#id').html(ord_id);
        $.ajax({
          url: '/custallorder/' + customer ,
          type: 'get',
          dataType: 'json',
          success: function(response) {
           ...
           // alert(len);
            ...
            if (len > 0) {
              ...
              var allord_row = "";
              var allord_header = '<div class="card-body"><div class="table-responsive"><table class="allord_details table table-bordered table-hover"><thead><tr><th class="text-center">Product ID</th><th class="text-center">Description</th><th class="text-center">Quantity</th><th class="text-center">Estimated Price*</th></tr></thead><tbody>';
              for (var i = 0; i < len; i++) {
                ...
                allord_row = allord_row.concat('<tr><td class="text-center">' + SKU_ID + '</td><td class="text-center">' + SKU_DESCRIPTION  + '</td><td class="text-center">' + SKU_QUANTITY + '</td><td class="text-center">' + TOTAL_SKU_VALUE + '</td></tr>');


              }
              
              var allord_tail = '</tbody></table></div></div>';
              var allord_info = allord_header.concat(allord_row).concat(allord_tail);
              $('#allord_detail').html(allord_info);
            ...
            $('table.allord_details').DataTable({
              "paging": true,
              "pageLength": 4,
              "lengthMenu": [
                [4, 10, -1],
...
//   data convert the pluck result in array
            var emp = table.rows({selected: true}).data().pluck(6).toArray();
            var flag = table.row({
              selected: true
            }).data()[7];
            var flags = table.rows({
              selected: true
            }).data().pluck(7).toArray();
            
            //const allEqual = arr => arr.every(val => val === arr[0]);
            function checkFlag(valueFlag) {
              return valueFlag == 0;
            }
  //    Racing flags are traditionally used in auto racing and similar motorsports to indicate track
  //after successfully activated it will redirect the adminn store
            ...
            //   otherwise it will show the warning for deactivated value
            } else {
        ...
        //  on click dunction in the btnSelectedRow class
    ...
    //    iT return the flag value to chek
            function checkFlag(valueFlag) {
              return valueFlag == 1;
            }

            if (flags.every(checkFlag)) {
        // Update the post as a token and showing deactivated message and reload it
              $.ajax({
                type: 'POST',
                url: "{{ route('adminn.store') }}",
                headers: {
            ...
            //   else it will show a notification popup.
            } else {
...
// it is use for authorization data table
    ...
    //   this is access for the download btn
        dom: 'l<"toolbar">Bfrtip',
        buttons: [{
            extend: 'excelHtml5',
            text: '<i style="float:right;" class="fas fa-download"></i>',
...
// this is for the screen dataTable
      $('#screen').DataTable({
               //  specify whether the paging of the DataTable is enabled
        "paging": true,
        //specify the number of rows of the table that are to be displayed on one page
...
//  this use for the component data table as a component and store the value in excel file format
      $('#component').DataTable({
        //  specify whether the paging of the DataTable is enabled
        "paging": true,
        //specify the number of rows of the table that are to be displayed on one page
...
// this is for the module table format data and update the data
      $('#module').DataTable({
        //  specify whether the paging of the DataTable is enabled
        "paging": true,
        //specify the number of rows of the table that are to be displayed on one page
...
//initComplete option is used to specify the function that will be invoked when the DataTable has fully loaded all the data.
        initComplete: function() {
          var btns = $('.dt-button');
          btns.addClass('btn btn-info');
          btns.removeClass('dt-button');
...
// here the len value is >0 then store the len value increment
            try {
              if (len > 0) {
                for (var i = 0; i < len; i++) {
...
//  here the vabiable tab is stored the customer data
      var tab;
...
//  One useful aspect of bracket notation is that it can be used as grab()
        function grab() {
          /* Promise to make sure data loads */
          return new Promise((resolve, reject) => {
            $.ajax({
...
// this shows the data chart define by the days and the data length
        $(document).ready(function() {
          grab().then((data) => {
            //console.log('Recieved our data', data);
            let regions = [];
...
// this is for the bargraph shows as a chartdata
              let ctx = $("#Charts");
...
//   this is use to handles errors in catch to destroy the bar graph which is shown previously
          }).catch((error) => {
            console.log(error);
          })
          barGraph.destroy();
...
// it gives the access to download the data as  a excel file
        buttons: [{
            extend: 'excelHtml5',
            text: '<i style="float:right;" class="fas fa-download"></i>',
            titleAttr: 'Excel'
...
// The initComplete option is used to specify the function that will be invoked when the DataTable has fully loaded all the data.
        initComplete: function() {
          ...
          btns.removeClass('dt-button');
          $("div.toolbar")
            .html('<div class="btn-group"><button type="button" class="btn btn-info btn-sm" title="Activate">Action</button><button type="button" class="btn btn-info btn-sm dropdown-toggle" data-toggle="dropdown"><div class="dropdown-menu" role="menu"><a class="dropdown-item" id="btnSelectedRows">Approve</a><div class="dropdown-divider"></div><a class="dropdown-item" id="btnSelectedRow">Reject</a></div></button></div>');
    ...
    //   this function is for checks the flag
            //const allEqual = arr => arr.every(val => val === arr[0]);
            function checkFlag(valueFlag) {
              return valueFlag == 0;
            }
...
// this is use to store the employee daat table inside the employee
      var ta;
...
// The initComplete option is used to specify the function that will be invoked when the DataTable has fully loaded all the data.
  ...
  {{-- This is use for the ALL EMployee tab inside USER and the blade file is employee.blade.php --}}
  <script>
  ...
  <script>
    $(function() {
      ...
      $("body").on("click", ".view_data", function(event) {
        ...
        //  Set the data in obj means it contain the all data for individual
        var obj = JSON.parse(vals);
        //alert(vals);
        var ownership;
        ...
        // Code to check whether the selected user is D user or normal user  for REPORTING_MANAGEER and APPROVER//
        if (obj.EMP_ID.startsWith("D") && !isNaN(obj.EMP_ID.substr(1, 4))) {
          document.getElementById('rep_mgr').disabled = false;
          document.getElementById('emp_approver').disabled =false ;
        }
        // IF the users is D USERS then It will show else part means the dropdown is Enable
        else{
          document.getElementById('rep_mgr').disabled = true;
          document.getElementById('emp_approver').disabled = true;
        }
        ...
        // This is for the Update btn inside Edit Employee Details for the employee_form
      $('#employee_form').on("submit", function(event) {
        ...
        //   After Update it gives Successful message
          success: function(data) {
            //alert(data.success);
	...
	// $("body").on("click", ".mybeat", function(event) {
  ...
  //        $.ajax({
	// 	  url: '/createbeatplan/' + myplanid + '/' + myplanname,
  //         type: 'get',
  //         dataType: 'json',
  //         success: function(response) {
  ...
  //             leng = response['datas'].length;
  //           }

  //           if (leng > 0) {
  //             var mycust_row = "";
  //             var mycust_header = '<div class="card-body"><div class="table-responsive"><table class="myplan_details table table-bordered table-hover"><thead><tr><th>Customer Name</th><th>City</th><th>Status</th></tr></thead><tbody>';
  //             for (var i = 0; i < leng; i++) {
  //               var CUST_NAME = response['datas'][i].CUST_NAME;
  //               var CUST_CITY = response['datas'][i].CUST_CITY;
  //                var FLAG = response['datas'][i].FLAG;
  //               mycust_row = mycust_row.concat('<tr><td>' + CUST_NAME + '</td><td>' + CUST_CITY + '</td><td>' + FLAG + '</td></tr>');


  //             }
  //             var mycust_tail = '</tbody></table></div></div>';
  //             var mycust_info = mycust_header.concat(mycust_row).concat(mycust_tail);

  //            $('#beatplan_detail').html(mycust_info);
  //             $('#mybeat').modal("show");
  //           } else {
  //             toastr.warning("No records found")
  //             $('#mybeat').modal("hide");
  ...
  //           $('table.myplan_details').DataTable({
  //             "paging": true,
  //             "pageLength": 4,
  //             "lengthMenu": [
  //               [4, 10, -1],
...
//  this is for thee  login detaills on click function
...
// this is for the order by the id onclick function
...
// // it is for the assign region it shows the assign region with their details
...
// this is fro the reorders on click functin
      $("body").on("click", ".reorders", function(event) {
        ...
        $('#total_val').html(code);
//    It show  the datafor regionorder  from id
        $.ajax({
      ...
      $("body").on("click", ".codes", function(event) {
      ...
      $("body").on("click", ".images", function(event) {
        ...
        var path = "http://127.0.0.1:8000/storage/" + imagess.IMAGE + "";
    ...
    $(function() {
      ...
      $("body").on("click", ".claimDay", function(event) {
        ...
        $('#claimdate').datetimepicker({
          format: 'L',
          minDate: new Date(currentYear, currentMonth, currentDate - claim_limit),
          maxDate: new Date()
        });
  ...
  {{-- if the id is authenticate then it hos aftter click the btn for create claim --}}
  @php
  $claim = Auth::id();
  @endphp
  <script>
...
// this is used for create claim part
...
//insert_form is the id of Create Module tab inside Authorization
//And it is for the Update inside edit btn in the Action bar
...
//screen_form is the id of Create Screen tab inside Authorization
//And it is for the Update inside edit btn in the Action bar
...
// This is the submit form for my customer edit btm inside action
    $('#mycust_form').on("submit", function(event) {
      ...
      $.ajax({
        ...
        // this is sending the edited data ti api
        data: {
          '_method': 'PUT',
          cust_name: cust_name,
          cust_mob: cust_mob,
...
//this is for customer order
$("body").on("click", ".corders", function(event) {
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            if (len > 0) {
              var corder_row = "";
              var corder_header = '<div class="card-body"><div class="table-responsive"><br><table class="corder_details table table-bordered table-hover"><thead><tr><th></th><th class="text-center">Product ID</th><th class="text-center">Quantity</th><th class="text-center">Estimated Price <span class="asterisk">*</span></th><th></th><th></th></tr></thead><tbody>';
              for (var i = 0; i < len; i++) {
                var SKU_ID = response['data'][i].SKU_ID;
                var SKU_QUANTITY = response['data'][i].SKU_QUANTITY;
                var TOTAL_SKU_VALUE = response['data'][i].TOTAL_SKU_VALUE;
                var STATUS = response['data'][i].STATUS;
                corder_row = corder_row.concat('<tr> <td></td><td class="text-center">' + SKU_ID + '</td><td class="text-center">' + SKU_QUANTITY  + '</td><td class="text-center">' + TOTAL_SKU_VALUE + '</td><td class="text-center">' + 1 + '</td><td class="text-center">' + 2 + '</td></tr>');


              }
              var corder_tail = '</tbody></table></div></div>';
              var corder_info = corder_header.concat(corder_row).concat(corder_tail);
              $('#corder_detail').html(corder_info);
           ...
           var t;
           t= $('table.corder_details').DataTable({
...
// The initComplete option is used to specify the function that will be invoked when the DataTable has fully loaded all the data.
            ...
            btns.removeClass('dt-button');
            $("div.toolbar")
           .html('<div><div class="btn-group"><button type="button" class="btn btn-info btn-sm" style="float:right;" title="Activate">Action</button><button type="button" class="btn btn-info btn-sm dropdown-toggle" data-toggle="dropdown"><div class="dropdown-menu" role="menu"><a class="dropdown-item " id="partial_approve"  >Partial Approve</a><div class="dropdown-divider"></div><a class="dropdown-item" id="approve_all">Approve</a><div class="dropdown-divider"></div><a class="dropdown-item" id="reject_all">Reject</a></div></div><br><div><input type="checkbox" id="checked">&nbsp;&nbsp;Select Checkbox to Approve Partially</input></div>');
            ...
            $('#checked').on('click', function() {                  
             t.column( 0 ).visible( true )
              ...
              // button for partial approving
              $('#partial_approve').on('click', function() {
              //order_id which has to update
                var orderr_id= document.getElementById("ord_id").value;

              // selected plant_id from drop-down
                var plant_id = document.getElementById("plant_id").value;
                if(plant_id===''){
                    toastr.warning('Please Select Plant')
                }
                else{
               // array of checked sku as approve
                var checked_product = t.rows({
                selected: true
                }).data().pluck(1).toArray();

               //array of checked status i.e 1
                var status_for_checked = t.rows({
                selected: true
                }).data().pluck(4).toArray();

                //console.log(status_for_checked);
                //array of unchecked sku as rejected
                var unnchecked_product = t.rows({
                selected: false
                }).data().pluck(1).toArray();

               // array of unchecked status for rejected status i.e 2
                var status_for_unchecked = t.rows({
                selected: false
                }).data().pluck(5).toArray();

               // push the unchecked sku in checked sku array 
                checked_product.push(...unnchecked_product);
                console.log(checked_product);

                // push the unchecked status in checked status array
                status_for_checked.push(...status_for_unchecked);
                console.log(status_for_checked);
               // alert(checked_product);
               
               // a variable with having the value of order_id 
                 var partial = orderr_id;
                 $.ajax({
                          type: 'POST',
                          url: '/corder/'+ orderr_id + '/' + partial,
                          headers: {
                                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                                   },
                  data: {
                          prooduct: checked_product,
                          statuses:status_for_checked,
                          plant_id: plant_id
                        },
                          success: function(data) {
                          toastr.success('Ordered Updated Successfully')
                          location.reload();
                                            }});
               } });
             ...
             $('#approve_all').on('click', function() {
               //order id for updation
               var orderr_id= document.getElementById("ord_id").value;

               // reads selected plant_id
               var plant_id = document.getElementById("plant_id").value;
               ...
               // a array of all sku in table 
               var approve_product = t.rows({
                selected: false
                }).data().pluck(1).toArray();
                console.log(approve_product);
               
               // a array of status of all sku 
                var status_for_all = t.rows({
                selected: false
                }).data().pluck(4).toArray();
            
                console.log(status_for_all);
            
              // a variable which has order id 
              var partial = orderr_id;
              var partiall = orderr_id;
              var all = orderr_id;
              var a='4';
              $.ajax({
                      type: 'POST',
                      url: '/corder/'+ orderr_id +'/'+ a,
                      headers: { 
                           'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                          },
                      data: {
                        prooduct: approve_product,
                        statuses:status_for_all,
                         plant_id: plant_id
                          },
                         success: function(data) {
                         toastr.success('Ordered Updated Successfully')
               ...
               // a array of all sku in table 
               var rejected_product = t.rows({
                selected: false
                }).data().pluck(1).toArray();
                console.log(rejected_product);
               
               // a array of status of all sku 
                var status_all = t.rows({
                selected: false
                }).data().pluck(5).toArray();
     ...
     $("body").on("click", ".allord", function(event) {
        var orders = $(this).attr("id");
        var valuess = document.getElementById(orders).getAttribute("value");
        var editss = JSON.parse(valuess);
        $('#orderr_id').val(editss.ORDER_ID);
...
$('#acknowledge_form').on("submit", function(event) {
      event.preventDefault();
      var orderss = document.getElementById("orderr_id").value;
      var pqr=orderss;
      var status = document.getElementById("acknowledge_status").value;
     //alert(status);
        ...
        success: function(data)
         {
          //alert(data.success);
          //console.log(data);
          toastr.success('Acknowledge Updated Successfully')
      ...
      $("body").on("click", ".corders", function(event) {
        ...
        var valuess = document.getElementById(corder).getAttribute("value");
        var edits = JSON.parse(valuess);
        $('#ord_id').val(edits.ORDER_ID);
        $('#plant_id').val(edits.PLANT_ID);
        $("#plant_id").select2();
       if (edits.APPROVE_STATUS == '1') {
       $("input[data-bootstrap-switch]").each(function() {
      $(this).bootstrapSwitch('state', true);
      });
...
$('#edit_form').on("submit", function(event) {
      event.preventDefault();
      var corder = document.getElementById("ord_id").value;
      //alert(corder);
      var plant_id = document.getElementById("plant_id").value;
      //alert(plant_id);
      var flag = document.getElementById("flag").value;
...
// this is used for submit the assign plant form
    $('#assign_plt_form').on("submit", function(event) {
// preventDefault() method is used to prevent the browser from executing the default action of the selected element.
      event.preventDefault();
      // store the lant id value inside assignplant
      var assignplant = document.getElementById("plt_Id").value;
    //   FormData interface provides a way to easily construct a set of key/value pairs representing form fields and their values
      let formData = new FormData(this);
    // ajax() method is used to request All jQuery
      $.ajax({
        type: 'POST',
  ...
  <script>
    //   It is used to specify the function to run after the document is loaded
    $(document).ready(function() {
// To make the clicked tab active in the navigation bar,
      $('.nav-link').on("click", function() {
// removeClass() Method. This method removes one or more class names from the selected elements.
        $('.nav-link').removeClass('active');
// it targets all the elements with class name . tab
        $(this).addClass('active');
//to find all siblings elements of the selected element. removeClass() method is used to remove one, multiple, or all the class names from the class attributes of the matched elements
...
//    it used to make a function available after the document is loaded.
//  bsCustomFileInput.init(); here init used to Finds your Bootstrap custom file input and will make them dynamic.
    $(document).ready(function() {
