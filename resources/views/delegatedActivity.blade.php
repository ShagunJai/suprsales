...
<section class="content-header">
	 ...
	 @isset($task_det)
	  ...
	  <form class="form-horizontal" id="delegated_activity_form" method="POST">
	        @csrf
            @method('PUT')
                <div class="card-body">
          ...
          <div class="col-sm-8">
            <select class="select2"  name="assignment_id" id="assignment_id" data-dropdown-css-class="select2-teal">
        ...
        <div class="modal-dialog">
          <div class="modal-content" style="width: 600px; height: 100px;">
              ...
              <!-- /.card-header -->
              <!-- form start -->
      <form role="form-horizontal" action="{{route('assignactivity.store')}}" method="POST">
                {{ csrf_field() }}
                <div class="card-body">
				...
				<div class="form-group row">
				 ...
				 <div class="form-group row add_activitys">
                                        <label class="col-sm-4 col-form-label">Start Date</label>
                                        <div class="col-sm-8">
                                              <div class="input-group date" id="startdates" data-target-input="nearest">
										<input type="text" name="startdate" class="form-control datetimepicker-input" data-target="#startdates" placeholder="Start Date" required />
      ...
      <form role="form-horizontal" action="{{route('delegatedactivity.store')}}" method="POST" enctype="multipart/form-data">
                {{ csrf_field() }}
                <div class="card-body">
				<input type="hidden" name="task_id" value="{{$taskID}}">
				<div class="form-group row">
    ...
    <div class="panel panel-default">
	  ...
	  @csrf
          ...
          <label class="col-sm-4 col-form-label">Assign</label>
          <div class="col-sm-8">
            <select class="select2" multiple="multiple" name="team_mem[]" id="team_mem" data-placeholder="Select Employees" data-dropdown-css-class="select2-teal">
                 @isset($emp)
				@foreach ($emp as $acts)
				<option value="{{ $acts['EMP_ID'] }}">{{ $acts['EMP_NAME'] }}</option>
				@endforeach 
						...
						@if($loop->last)
							@if($del['DELIGATED_TO'] == $login_id)	
								@if($currentActivity!= $previousActivity)
									...
									@endif
							@else
								@if($del['DELIGATION_FROM'] == $login_id)
										@if($currentActivity!= $previousActivity)
												@php
													$delegated_counter = $delegated_counter + 1;
													array_push($delegatedActivitiesArray, $currentActivity);
...
@endforeach	
	   ...
	   <div class="card card-success">
			...
			{{$assignedByDelegation_counter}} Assigned Activities via Delegation
			</h3>
			...
			@if(in_array($actID, $assignedByDelegationActivitiesArray))	 
        ...
        <h3 class="card-title text-primary">{{$act['ACTIVITY_HEADER']}}</h3>
		 ...
		 <span class="float-right">
					 ...
					 <a class='btn btn-warning btn-sm delegated_modal' href='#' value="{{$auths}}" id="{{$act['ACTIVITY_ID']}}" title="Edit">
    ...
    <div class="panel panel-default">
	  ...
	  @endif
		...
		<div class="card card-success">
            ...
            <h3 class="card-title">
			@php 
			$total_other = $delegated_counter + $others_counter + $assigned_counter;
			@endphp
            ...
            <div class="card-tools">
			  <button type="button" class="btn btn-tool" data-card-widget="collapse" data-toggle="tooltip" title="Expand">
                <i class="fas fa-minus"></i></button>
				 ...
				 <span class="badge badge-info col">{{$assigned_counter}} Assigned Activities</span>
        ...
        <tr>
			   ...
			   @foreach ($value['ACTIVITY'] as $act)	
			   @if(in_array($act['ACTIVITY_ID'], $assignedActivitiesArray))
						@php
							$task = "Assigned";
						@endphp
