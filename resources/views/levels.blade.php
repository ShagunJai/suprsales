...
{{-- It compare the Authencate users if the users is authorize then it will continue the next level --}}
  ...
  <div class="wrapper">
    ...
    {{-- This is the home tab in the NAVBAR --}}
    ...
    <!-- SEARCH FORM -->
    {{-- In the NAVBAR there is a search option for all pages  --}}
  ...
  @endphp
      ...
      {{-- All the announcement is shows here in the navbar icon --}}
      <li class="nav-item">
      <form action="/announce" method="GET">
       ...
       {{-- It gives the The Announcement dropdown after clicking announcement icon is there is any announcement it will show otherwise see all announcementwill shown in the dropdown --}}
        <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
      ...
      {{-- It give the users name ,Image and EMail in the top right corner --}}
	  @php
  $username = Auth::user()->name;
  $image = Auth::user()->image;
  $email = Auth::user()->email;
  @endphp
   {{-- After clicking the picture of the navbar it gives dropdown with name image and email --}}
...
@php
	  ...
	  @if($counters==0)
		 ...
		 @endforeach
			...
			@foreach ($ann as $val)
             @if($val['auth_reference'] == 'stock' || $val['auth_reference'] == 'asales' || $val['auth_reference'] == 'corder'|| $val['auth_reference'] == 'packing' || $val['auth_reference'] == 'plantview')
		   <li class="nav-item has-treeview">
	   @if($counterss==0)
            <a href="" class="nav-link">
...
{{-------------------------------------------------------------------------}}
	   ...
	   @if($countyr==0)
		  ...
		  @endif
			...
			@foreach ($ann as $val)
             @if($val['auth_reference'] == 'ann' || $val['auth_reference'] == 'anns' ||$val['auth_reference'] == 'newproductlaunch')
          <li class="nav-item has-treeview">
	  @if($countersss==0)
            <a href="" class="nav-link">
              <i class="nav-icon fas fa-bullhorn"></i>
			...
			@if($val['auth_reference'] == 'ann')
              <li class="nav-item">
                <a href="http://127.0.0.1:8000/ann" class="nav-link">
                  <i class="nav-icon fas fa-plus-square"></i>
                  <p>Create</p>
                </a>
              </li>
              ...
              <li class="nav-item">
                <a href="http://127.0.0.1:8000/anns" class="nav-link">
                  <i class="nav-icon fas fa-plus-square"></i>
                  <p>Create Type</p>
                </a>
              </li>
